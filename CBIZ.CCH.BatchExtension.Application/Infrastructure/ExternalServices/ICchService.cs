...
{
    Task<Either<string , BatchExtensionException>> CreateBatchAsync(Guid queueId, string returnType, List<string> returnIds, CancellationToken cancellationToken = default);
    Task<Either<(List<BatchItemStatus> items, string status) , BatchExtensionException>> GetBatchStatusAsync(Guid executionId, int returnsCount, CancellationToken cancellationToken = default);
